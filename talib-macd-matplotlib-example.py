...
sec_id = data.get_data_robinhood(ticker, '2018-06-01')
...
sec_id.rename(columns={'close_price': 'Close', 'high_price': 'High', 'low_price': 'Low', 'open_price': 'Open'},
              inplace=True)
print(sec_id)
...
close_real_data = np.array(sec_id.Close.values, dtype='f8')
analysis['rsi'] = ta.RSI(close_real_data, RSI_PERIOD)
# analysis['sma_r'] = pd.rolling_mean(analysis.rsi, RSI_AVG_PERIOD)
analysis['sma_r'] = analysis.rsi.rolling(RSI_AVG_PERIOD).mean()
# check shift

analysis['macd'], analysis['macdSignal'], analysis['macdHist'] = ta.MACD(close_real_data, fastperiod=MACD_FAST,
                                                                         slowperiod=MACD_SLOW, signalperiod=MACD_SIGNAL)
high_real_data = np.array(sec_id.High.values, dtype='f8')
low_real_data = np.array(sec_id.Low.values, dtype='f8')
analysis['stoch_k'], analysis['stoch_d'] = ta.STOCH(high_real_data, low_real_data,
                                                    close_real_data, slowk_period=STOCH_K,
...
mpf.candlestick2_ochl(ax1, sec_id['Open'], sec_id['Close'], sec_id['High'], sec_id['Low'], width=0.5, colorup='r',
